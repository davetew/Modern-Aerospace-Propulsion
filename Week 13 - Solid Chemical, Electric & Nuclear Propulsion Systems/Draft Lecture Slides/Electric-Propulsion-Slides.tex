...
\begin{frame}{Exhaust Molecular Weight ($\mathfrak{M}$) Differerences}
  \draft
    Liquid hydrogen yields the lightest possible exhaust products, while solids are burdened by heavy metal oxides and chlorine species.
...
\begin{frame}{Factor 2: Two-Phase Flow Losses}
  \draft
    Solid propellants use Aluminum (Al) to increase combustion temperature, producing condensed-phase Aluminum Oxide ($Al_2O_3$).
    
    \begin{columns}
        \column{0.5\textwidth}
        \textbf{The Physics of Drag:}
        \begin{itemize}
            \item $Al_2O_3$ particles (liquid/solid) cannot expand to do pressure work.
...
\begin{frame}{The Trade-off: Density Impulse}
  \draft
    Solids lose on Specific Impulse ($I_{sp}$) but win on Density Impulse ($I_{sp} \times \rho$).
...
\begin{frame}{Summary}
  \draft
    \begin{table}
        ...
        \begin{tabular}{l c c}
            \toprule
            \textbf{Parameter} & \textbf{Solids (APCP)} & \textbf{Liquids (Hydrolox)} \\
            \midrule
            \textbf{Combustion Temp ($T_c$)} & $\approx 3400$ K & $\approx 3600$ K \\
            \textbf{Exhaust MW ($\mathfrak{M}$)} & \alert{High (~30)} & \alert{Low (~12)} \\
    ...
    \vspace{1em}
    \textbf{Conclusion:} Liquids achieve higher $I_{sp}$ primarily due to the low molecular weight of Hydrogen, while solids suffer from heavy exhaust products and particle drag losses.
\end{frame}
