\documentclass[aspectratio=169]{beamer}

% Theme settings
\usetheme{Madrid}
%\usecolortheme{beaver}
\usecolortheme{Columbia}
\setbeamertemplate{navigation symbols}{}
\setbeamertemplate{itemize items}[circle]

% Packages for Math and Graphics
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{bm} % For bold math symbols (vectors/tensors)
\usepackage{graphicx}
\usepackage{svg}
\usepackage{booktabs}
\usepackage[table]{xcolor}
\usepackage{tikz}
\usepackage{pgfplots}
\pgfplotsset{compat=1.18}
\usetikzlibrary{decorations.pathmorphing,snakes,patterns,arrows.meta, shapes, positioning, calc}
\usepackage{hyperref} % For clickable URLs and escaping underscores via \url

% Global upper-right logo (adjust path & height as needed)
% Place your logo image at figs/logo.png or change path below.
%\newcommand{\CornerLogo}{%
%  \begin{tikzpicture}[remember picture,overlay]
%    \node[anchor=north east] at ([xshift=-2mm,yshift=-2mm]current page.north east){\includegraphics[height=0.9cm]{figs/logo.png}};%
%  \end{tikzpicture}%
%}
% Inject logo onto every slide (including title) via background template.
%\addtobeamertemplate{background}{}{\CornerLogo}

% Define Draft Watermark
\newcommand{\draft}{%
  \begin{tikzpicture}[remember picture, overlay]
    \node[rotate=45, scale=8, text opacity=0.1, text=red] at (current page.center) {DRAFT!};
  \end{tikzpicture}
}

% Package for Notes
%\usepackage{pgfpages}
%\setbeameroption{show notes on second screen}

% Title Information
\title[Modern Aerospace Propulsion]{Solid Chemical \& Electric Propulsion Systems}
\subtitle{Physics, Engineering, and System Integration}
\author{David Tew}
\institute{Columbia University}
\date{\today}

\begin{document}

%---------------------------------------------------------
% Title Slide
%---------------------------------------------------------
\begin{frame}
  \titlepage
\end{frame}

%---------------------------------------------------------
% Outline
%---------------------------------------------------------
\begin{frame}{Lecture Outline}
  \tableofcontents
\end{frame}

%---------------------------------------------------------
% Section 1: Solid Chemical Propulsion
%---------------------------------------------------------
\section{Solid Chemical Propulsion}

\begin{frame}{Solid Chemical Propulsion Overview}
  \framesubtitle{\textit{Energy dense, storable, and instantly available}}
  \begin{block}{Solid Chemical Propulsion Systems:}
    Solid chemical propulsion offers high volumetric energy density, long-term storability, and instant readiness.  Their scale
    ranges from micro-scale thrusters for attitude control to large boosters for heavy-lift launch vehicles.
  \end{block}
  \vspace{0.5cm}
  \textbf{Critical Physics:}
  \begin{itemize}
    \item Saint-Robert's Law ($r_b=aP^n_c$) -- burning rate dependence on chamber pressure.
    \item Heterogeneous combustion physics -- with multiple reacting phases.
    \item Erosive burning regimes in high $L/D$ motors.
  \end{itemize}
\end{frame}

\begin{frame}{Example Composite Propellant ($NH_4ClO_4$/HTPB/Al)}
  \framesubtitle{\textit{Ammonium Perchlorate, Hydroxyl-Terminated Polybutadiene, \& Aluminum}}

  \begin{columns}
    \column{0.65\textwidth}
    \textbf{Constituents \& Roles:}
    \begin{itemize}
      \item \textbf{Oxidizer:} $NH_4ClO_4$ (AP)
      \item \textbf{Binder:} HTPB. Structure + fuel source via pyrolysis.
      \item \textbf{Fuel:} Aluminum. $T_{flame} \approx 3400K$; acoustic damper.
    \end{itemize}

    \column{0.35\textwidth}
    \centering

    % TikZ Diagram: APCP Surface Structure
    \begin{tikzpicture}[scale=0.7, transform shape]
      % Propellant Bulk
      \fill[gray!20] (0,0) rectangle (6,3);
      \node at (3,0.5) {Solid Propellant (HTPB Matrix)};
      % AP Crystals
      \draw[fill=white] (1,2) circle (0.6);
      \node at (1,2) {AP};
      \draw[fill=white] (3,2.5) circle (0.4);
      \node at (3,2.5) {AP};
      \draw[fill=white] (5,1.8) circle (0.7);
      \node at (5,1.8) {AP};
      % Al Particles
      \draw[fill=gray] (2,1.5) circle (0.2);
      \draw[fill=gray] (4,1.2) circle (0.2);
      \node at (4.5, 1.2) {\small Al};
      % Burning Surface
      \draw[thick, decorate, decoration={snake, amplitude=1mm}] (0,3) -- (6,3);
      % Flame Zone
      \shade[bottom color=yellow, top color=red, opacity=0.6] (0,3) rectangle (6,5);
      \node at (3, 4) {\textbf{Primary Diffusion Flame}};
      % Al Agglomeration
      \draw[fill=gray!80, draw=black] (2, 3.2) circle (0.3);
      \draw[->, thick] (2, 3.5) -- (2.5, 4.5);
      \node[right] at (2.5, 4.5) {Al Agglomerate Ignition};
    \end{tikzpicture}
  \end{columns}

  \vspace{1em}
  \textbf{Sequential Combustion Mechanism:}
  \begin{enumerate}
    \item AP phase change (orthorhombic to cubic)
    \item HTPB binder pyrolysis $\rightarrow$ gaseous HC fuels + solid char
    \item AP Decomposition: $NH_4ClO_4 \longrightarrow NH_3 + HClO_4 \longrightarrow Cl_2 + O_2 + H_2O + N_2$
    \item Surface diffusion flame
    \item Al Melting \& Combustion (far from surface).
  \end{enumerate}

\end{frame}
  
%==================================================================
% --- Sub-Section: Internal Ballistics ---
\subsection{Internal Ballistics -- 0-D SRM Model}

\iffalse
\begin{frame}{Combustion Chamber Control Volume ($V_c$) Analysis}
  \begin{itemize}
    \item Gaseous Mass Conservation:
    \begin{align*}
      \frac{d}{dt}(\rho_g V_c) &= \dot{m}_{gen} - \dot{m}_{out} \\
      V_c\frac{d\rho_g}{dt} + \rho_g \frac{dV_c}{dt} &= \dot{m}_{gen} - \dot{m}_{out}
    \end{align*}
    \item $V_c$ increases as the grain burns:
    $$\frac{dV_c}{dt} = A_b r_b$$
    \item $A_b$ is the burning surface area, and $r_b$ is the burn rate.
    \vspace{0.2em}
    \item Mass generation rate from propellant burning: $\dot{m}_{gen} = \rho_p A_b r_b$
    \vspace{0.2em}
    \item Mass outflow rate through nozzle: $\dot{m}_{out} = \frac{P_c A_t}{c^*}$
  \end{itemize}
\end{frame}

\begin{frame}{Unsteady Pressure Equation}
  \textbf{Goal:} Derive an expression for $dP_c/dt$ in terms of chamber properties and burn rate.
  \begin{align*}
    V_c\frac{d\rho_g}{dt} + \rho_g \frac{dV_c}{dt} &= \dot{m}_{gen} - \dot{m}_{out} \\
    \frac{V_c}{RT_c}\left[ \frac{dP_c}{dt} - \underbrace{\frac{P_c}{T_c}\frac{dT_c}{dt}}_{\text{Assume } \frac{dT_c}{dt}=0} \right] &= \rho_p A_b r_b - \frac{P_c A_t}{c^*} - \rho_g \frac{dV_c}{dt}\\
    \frac{dP_c}{dt} &= \frac{RT_c}{V_c} \left[ \rho_p A_b r_b \left( 1 - \frac{\rho_g}{\rho_p}\right) - \frac{P_c A_t}{c^*} \right]
  \end{align*}
\end{frame}
\fi

\begin{frame}{Combustion Chamber Control Volume ($V_c$) Analysis}
  \framesubtitle{\textit{Predict 0-D chamber conditions, thrust \& $I_{sp}$ versus time}}
  \begin{itemize}
    \item Control volume $\Rightarrow$ gas inside the combustion chamber.
    \item Gas assumed perfectly mixed and spatially uniform.
    \item Ideal gas: $pV = mRT$.
  \end{itemize}

  \begin{columns}
    \column{0.45\textwidth}
    \begin{itemize}
      \item Sources:
      \begin{itemize}
        \item Propellant–generated mass $\dot m_b$
        \item Igniter mass $\dot m_{\text{ign}}$
        \item Combustion heat release $\dot Q_{\text{comb}}$
      \end{itemize}
      \item Sinks:
      \begin{itemize}
        \item Nozzle mass flow $\dot m_e$
        \item Wall heat losses $\dot Q_{\text{wall}}$
      \end{itemize}
    \end{itemize}

    \column{0.55\textwidth}
    \centering
    \begin{tikzpicture}[scale=0.9]
      % Chamber
      \draw[thick] (0,0) rectangle (5,2);
      \node at (2.5,1) {CV};
      % Burn surface
      \draw[fill=red!20] (-0.8,0) rectangle (0,2);
      \node[rotate=90] at (-0.4,1) {Grain};
      % Arrows
      %\draw[-{Stealth[length=3mm]}] (-1.5,1) -- (-0.8,1);
      %\node at (-1.5,1.3) {$\dot m_b$};
      \draw[-{Stealth[length=3mm]}] (0,1) -- (0.7,1);
      \node at (1.1,1.4) {$\dot m_b$, $\dot Q_{\text{comb}}$};
      \draw[-{Stealth[length=3mm]}] (5,1) -- (6,1);
      \node at (6,1.3) {$\dot m_e$};
      \draw[-{Stealth[length=3mm]}] (2.5,3) -- (2.5,2);
      \node at (2.5,3.3) {$\dot m_{\text{ign}}$};
    \end{tikzpicture}
  \end{columns}
\end{frame}

\begin{frame}{Mass Balance}
  \framesubtitle{\textit{Unsteady mass conservation with propellant burn \& nozzle outflow}}
  \centering
  \begingroup
  \renewcommand{\arraystretch}{1.2}
  \rowcolors{1}{gray!10}{white}
  \resizebox{\textwidth}{!}{%
    \begin{tabular}{r l}
      \textbf{Unsteady mass conservation:} & $\dfrac{dm}{dt} = \dot m_b + \dot m_{\text{ign}} - \dot m_e$ \\
      Propellant mass generation: & $\dot m_b = \rho_p\, r_b\, A_b(s)$ \\
      Saint--Robert burn law: & $r_b = f_{\text{ign}}(t)\,a\,p^n$ \\
      Ignition ramp: & $f_{\text{ign}}(t) = 1 - \exp\!\left(-\dfrac{t - t_{\text{ign,start}}}{\tau_{\text{ign}}}\right)$ \\
      Choked nozzle mass flow: & $\dot m_e = C_d A_t p\, \sqrt{\dfrac{\gamma}{RT}}\,\left(\dfrac{2}{\gamma+1}\right)^{\frac{\gamma+1}{2(\gamma-1)}}$ \\
    \end{tabular}%
  }
  \endgroup
\end{frame}

\begin{frame}{Energy Balance with Heat Release}
  \framesubtitle{\textit{Unsteady energy conservation with combustion heat release \& wall losses}}
  Internal energy:
  $$
    U = m c_v T
  $$
  Full unsteady first law:
  $$
    \frac{d}{dt}(m c_v T)= \dot m_b h_b + \dot m_{\text{ign}} h_{\text{ign}} - \dot m_e h_e - p\frac{dV}{dt} + \dot Q_{\text{wall}} + \dot Q_{\text{comb}}
  $$
  Combustion heat release:
  $$
    \dot Q_{\text{comb}} = \dot m_b \Delta h_c
  $$
  Total heat term:
  $$
    \dot Q = \dot Q_{\text{wall}} + \dot Q_{\text{comb}} + \dot Q_{\text{ign}}.
  $$
\end{frame}

\begin{frame}{Deriving $\frac{dp}{dt}$ (Step 1)}
  \framesubtitle{\textit{Use the mass and energy equations to derive an expression for the evolution of chamber pressure with time.}}
  Start with:
  $$
    U = m c_v T
  $$
  $$
    \frac{d}{dt}(m c_v T) = \dot m_b h_b + \dot m_{\text{ign}} h_{\text{ign}} - \dot m_e h_e - p\frac{dV}{dt} + \dot Q
  $$
  Assume:
  $$
    h_b \approx h_e \approx c_p T, \qquad h_{\text{ign}} \approx c_p T_{\text{ign}}
  $$
  Left side:
  $$
    \frac{d}{dt}(mc_vT)= c_v\left(m\frac{dT}{dt} + T\frac{dm}{dt}\right)
  $$
  Right side becomes:
  $$
    c_p T\frac{dm}{dt} - p\frac{dV}{dt} + \dot Q
  $$
  Use mass balance:
  $$
    \frac{dm}{dt} = \dot m_b + \dot m_{\text{ign}} - \dot m_e
  $$
\end{frame}

\begin{frame}{Deriving $\frac{dp}{dt}$ (Step 2)}
  Rearrange:
  $$
    c_v m\frac{dT}{dt}= (c_p-c_v)T\frac{dm}{dt} - p\frac{dV}{dt} + \dot Q
  $$
  Use $c_p-c_v = R$:
  $$
    \frac{dT}{dt}= \frac{R T\frac{dm}{dt} - p\frac{dV}{dt} + \dot Q}{c_v m}
  $$
  Differentiate ideal gas:
  $$
    pV = mRT
  $$
  $$
    \frac{dp}{dt}V + p\frac{dV}{dt}= R\left(T\frac{dm}{dt} + m\frac{dT}{dt}\right)
  $$
  Substitute above $dT/dt$ expression.
\end{frame}

\begin{frame}{Deriving $\frac{dp}{dt}$ (Step 3)}
  Compute:
  $$
    R\left(T\frac{dm}{dt} + m\frac{dT}{dt}\right)= R T\frac{dm}{dt}+ \frac{R}{c_v}\left(R T\frac{dm}{dt} - p\frac{dV}{dt} + \dot Q\right)
  $$
  Use $\gamma = \frac{c_p}{c_v}$ and $\frac{R}{c_v}=\gamma-1$:
  $$
    = \gamma R T\frac{dm}{dt} - (\gamma-1)p\frac{dV}{dt}+ (\gamma-1)\dot Q
  $$
  Insert into:
  $$
    \frac{dp}{dt}V + p\frac{dV}{dt}= R\left(T\frac{dm}{dt} + m\frac{dT}{dt}\right)
  $$
  Thus:
  $$
    \frac{dp}{dt}V= \gamma RT\frac{dm}{dt} - \gamma p\frac{dV}{dt}+ (\gamma-1)\dot Q
  $$
\end{frame}

\begin{frame}{Deriving $\frac{dp}{dt}$ (Final Form)}
    Divide by $V$:
    $$
        \frac{dp}{dt}= \frac{\gamma RT}{V}\frac{dm}{dt} - \frac{\gamma p}{V}\frac{dV}{dt}+ \frac{\gamma-1}{V}\dot Q
    $$
    Use ideal gas $pV=mRT$:
    $$
        \frac{RT}{V}=\frac{p}{m}
    $$
    Therefore:
    $$
        \boxed{\frac{dp}{dt}= \gamma p\left[\frac{1}{m}\frac{dm}{dt}- \frac{1}{V}\frac{dV}{dt}\right]+ \frac{\gamma-1}{V}\dot Q}
    $$
\end{frame}

\begin{frame}{Volume Evolution and Web Regression}
  \framesubtitle{\textit{Chamber volume and propellant burn area calculated from propellant geometry model}}
    Web regression:
    $$
        \frac{ds}{dt} = r_b = f_{\text{ign}}(t)\,a\,p^n
    $$
    Grain geometry tables:
    $$
        A_b(s), \qquad V(s), \qquad \frac{dV}{ds}(s)
    $$
    Chamber volume evolution:
    $$
        \frac{dV}{dt} = \frac{dV}{ds}(s)\,\frac{ds}{dt}
    $$
    \centering
    \begin{tikzpicture}
        \draw[thick] (0,0) rectangle (5,2);
        \draw[fill=red!20] (-1,0) rectangle (0,2);
        \node[rotate=90] at (-0.5,1) {Grain};
      \draw[thick,->] (0.7,1) -- (0,1);
      \node at (0.7,1.3) {$r_b$};
        
    \end{tikzpicture}
\end{frame}

\begin{frame}{Ablative Nozzle Throat Erosion \& Cooling}
  \begin{alertblock}{Nozzle Cooling Challenge}
    \begin{itemize}
      \item Combustion gas temperature ($>3000K$) exceeds melting point of all metals. 
      \item Active cooling impractical due to solid grain geometry.
    \end{itemize}
  \end{alertblock}
  \vspace{0.5cm}
  \begin{columns}
    \column{0.65\textwidth}
    \textbf{Solution: Carbon-Carbon (C/C) Ablation}
    \begin{itemize}
        \item Mechanism is chemical erosion, not melting.
        \item Diffusion-controlled oxidation by $H_2O$ and $CO_2$.
        \item $C_{(s)} + H_2O_{(g)} \rightarrow CO_{(g)} + H_2$
    \end{itemize}
    \vspace{0.5cm}
    \textbf{Bartz Correlation Scaling:} $\dot{r}_{erosion} \propto P_c^{0.8} D_t^{-0.2}$ \\
    \vspace{1em}
    \textbf{Simple Alternative:} $\dot{r}_{erosion} = k_{\text{eros}} |\dot m_e|$

    \column{0.35\textwidth}
    % TikZ: Nozzle Ablation
    \centering
    \begin{tikzpicture}[scale=0.8]
        % Nozzle Wall Initial
        \draw[thick, fill=black!80] (0,0) -- (1, -1) -- (2, -1) -- (3, 0) -- (3, 1) -- (0, 1) -- cycle;
        \node[white] at (1.5, 0) {C/C Throat};
        
        % Erodable layer
        \draw[dashed, red, thick] (0.8, -0.8) -- (2.2, -0.8);
        \node[red, anchor=west] at (2.3, -0.8) {Recession $\dot{r}$};
        
        % Gas Flow
        \draw[blue, thick, ->] (-1, -1.5) -- (4, -1.5);
        \node[blue] at (1.5, -2) {High Speed Flow ($Mach\ge1$)};
    \end{tikzpicture}
  \end{columns}

\end{frame}

\begin{frame}{Final Pressure Equation (SRM Form)}
    With the SRM source terms:
    $$
        \frac{dm}{dt} = \dot m_b + \dot m_{\text{ign}} - \dot m_e,\qquad \frac{dV}{dt} = \frac{dV}{ds}(s)\frac{ds}{dt},
    $$
    the chamber pressure ODE is:
    $$
        \boxed{\frac{dp}{dt}= \gamma p\left[\frac{\dot m_b + \dot m_{\text{ign}} - \dot m_e}{m}- \frac{1}{V}\frac{dV}{dt}\right]+ \frac{\gamma - 1}{V}\dot Q}
    $$
\end{frame}

\begin{frame}{Final ODE System}
  \framesubtitle{\textit{System of four ordinary differential equations}}
    State vector:
    $$
        y(t)=\begin{bmatrix}p(t)\\[4pt]m(t)\\[4pt]s(t)\\[4pt]r_t(t)\end{bmatrix}
    $$
    ODEs:
    $$
        \frac{dp}{dt}= \gamma p\!\left[\frac{\dot m_b + \dot m_{\text{ign}} - \dot m_e}{m}- \frac{1}{V}\frac{dV}{dt}\right]+ \frac{\gamma - 1}{V}\dot Q
    $$
    $$
        \frac{dm}{dt} = \dot m_b + \dot m_{\text{ign}} - \dot m_e
    $$
    $$
        \frac{ds}{dt} = f_{\text{ign}}(t)\,a\,p^n
    $$
    $$
        \frac{dr_t}{dt} = k_{\text{eros}}|\dot m_e|
    $$
\end{frame}

\begin{frame}{Assumptions \& Limitations of the 0-D SRM Model}
    \textbf{Major assumptions:}
    \begin{itemize}
        \item Chamber gas is \textbf{perfectly mixed} (no spatial gradients).
        \item Propellant gases instantaneously reach chamber temperature.
        \item Ideal-gas thermodynamics: $pV=mRT$.
        \item Quasi-steady Saint–Robert burn law: $r_b = a p^n$.
        \item Choked nozzle flow with fixed $C_d$.
        \item Lumped heat loss: $\dot Q_{\text{wall}} = -h_wA_w(T-T_w)$.
        \item Grain geometry encoded through tables $A_b(s)$ and $V(s)$.
    \end{itemize}
    \vspace{1em}
    \textbf{Limitations:}
    \begin{itemize}
        \item Cannot predict combustion instabilities or pressure oscillations.
        \item No axial or transverse wave dynamics (1-D/3-D neglected).
        \item No local flame chemistry or finite-rate kinetics.
        \item No particle dynamics (Al droplets, slag accumulation).
        \item No two-phase flow in nozzle.
    \end{itemize}
    \centering
    \small
\end{frame}

\begin{frame}{SRM Model Implementation}
  \framesubtitle{\textit{Uploaded to Course Github Repository}}
  % Display screen shot of VS Code implementation
  \centering
  \includegraphics[width=0.8\textwidth]{figs/SRM-Model-Screenshot.png}
\end{frame}

\begin{frame}{Example SRM Simulation Results}
  \framesubtitle{\textit{Intended to be an approximate representation of the Space Shuttle's SRB}}
  \centering
  \includegraphics[width=0.8\textwidth]{figs/SRM-Model-Predictions.png}
\end{frame}

\begin{frame}{Saint-Robert's Law}
  \textbf{Saint-Robert's Law:} $r_b = a P_c^n$
    \begin{itemize}
        \item $a$ = temperature dependent coefficient (hotter grains burn faster).
        \item $n$ = pressure exponent
    \end{itemize}
  \begin{columns}
    \column{0.5\textwidth}
    %\centering
    \hspace{3cm}% shift block right by 1 cm
    \begin{block}{Stability Criterion ($\dot{m}_{gen} \propto P_c^n$, $\dot{m}_{out} \propto P_c$)}
      $$\begin{cases}
        n < 1 & \text{Stable}: \uparrow P_c \Rightarrow |\Delta\dot{m}_{gen}| < |\Delta\dot{m}_{out}| \\
        n > 1 & \text{Unstable}: \uparrow P_c \Rightarrow |\Delta\dot{m}_{gen}| > |\Delta\dot{m}_{out}|
      \end{cases}$$
    \end{block}
    \column{0.5\textwidth}
    \centering
    \begin{tikzpicture}[scale=0.6]
      \begin{axis}[
        width=0.95\linewidth,
        axis lines=left,
        xlabel={$time$},
        ylabel={$P_c$},
        xtick=\empty, ytick=\empty,
        xmin=0, xmax=10,
        ymin=0,
        clip=false
      ]
        \addplot[domain=0:10, thick, blue] {2*x^0.5};
        \node at (axis cs: 6, 4) [right, blue] {Stable ($n<1$)};
        \addplot[domain=0.1:4, thick, red, dashed] {0.5*x^2};
        \node at (axis cs: 3, 8) [right, red] {Unstable ($n>1$)};
      \end{axis}
    \end{tikzpicture}
  \end{columns}
\end{frame}

\iffalse
\begin{frame}{Steady State Chamber Pressure}
  \begin{itemize}
    \item \textbf{Assume Steady State:} 
    $$\frac{dP_c}{dt} = 0$$
    $$\rho_p A_b r_b \left( 1 - \underbrace{\frac{\rho_g}{\rho_p}}_{neglect}\right) = \frac{P_c A_t}{c^*}$$
    \item \textbf{Substitute Saint-Robert's Law ($r_b = a P_c^n$):}
          $$\rho_p A_b a P_c^n = \frac{P_c A_t}{c^*}$$
    \item \textbf{Equilibrium Pressure:}
          $$P_{c}^{eq} \approx \left[ \rho_p a c^* K_n \right]^{\frac{1}{1-n}}$$
          \item Note sensitivity to $K_n = A_b / A_t$. If $n=0.5$, $P \propto K_n^2$.
  \end{itemize}
\end{frame}
\fi

%==========================================================
% --- Sub-Section: Grain Design ---
\subsection{Grain Design}

\begin{frame}{Burn Area ($A_b$) Geometric Evolution}
  \textbf{Evolution of Grain Geometry ($A_b(t)$) Controls Thrust Profile:}
    \begin{itemize}
        \item \textbf{Progressive:} $A_b$ increases $\Rightarrow$ $T$ increases.
        \item \textbf{Neutral:} $A_b$ constant $\Rightarrow$ $T$ constant.
        \item \textbf{Regressive:} $A_b$ decreases $\Rightarrow$ $T$ decreases.
    \end{itemize}
    \vspace{1em}
    \textbf{Common Grain Geometries:}
    \begin{itemize}
        \item \textbf{Progressive:} Circular Perforation or Center-Perforated Cylinder
        \item \textbf{Neutral:} Star or Wagon Wheel
        \item \textbf{Regressive:} Outer-burning grains or multi-propellant stacks
    \end{itemize}
\end{frame}

\begin{frame}{Advanced Grain Geometries: Star and Finocyl}
    \begin{columns}
        \column{0.5\textwidth}
        \textbf{Star:}
        \begin{itemize}
            \item $A_b \approx $ constant (neutral burn). 
            \item Sliver formation an efficiency challenge.
        \end{itemize}
        \vspace{1em}

        \centering
        % TikZ: Grain Cross Sections
        \begin{tikzpicture}[scale=0.8]
          % Star Grain
          \node at (0, 2.5) {\textbf{Star Grain}};
          \draw[thick, fill=gray!30] (0,0) circle (2);
          % Six-pointed star (centered), alternating outer/inner radii
          \fill[white] (0:1.5) -- (30:0.5) -- (60:1.5) -- (90:0.5) -- (120:1.5) -- (150:0.5)
                      -- (180:1.5) -- (210:0.5) -- (240:1.5) -- (270:0.5) -- (300:1.5) -- (330:0.5) -- cycle;
          
          % Web Definition
          %\draw[red, <->] (60:1.5) -- (60:2);
          %\node[red, right] at (60:1.8) {Web ($w$)};
        \end{tikzpicture}

        \column{0.5\textwidth}
        \textbf{Finocyl (Fin-On-Cylinder):}
        \begin{itemize}
            \item Provides flexibility in thrust profile design.
            \item Shuttle SRM: High thrust after launch, lower thrust during max-Q.
        \end{itemize}

        % Use \url to safely typeset underscores in the link
        \centering\small\url{https://wikis.mit.edu/confluence/display/RocketTeam/Commercial+Grain+Geometries}

        \centering
        % TikZ: Grain Cross Sections
        % External Finocyl Image (replace placeholder path with your figure)
        % If the image file exists at figs/finocyl.png it will be included;
        % otherwise a framed placeholder box will appear as a reminder.
        \IfFileExists{figs/Shuttle\_SRB.jpg}{%
            \includegraphics[width=0.9\linewidth]{figs/Shuttle\_SRB.jpg}%
        }{%
            \fbox{\parbox{0.9\linewidth}{Finocyl image placeholder\\Add image at figs/Shuttle\_SRB.jpg}}%
        }
    \end{columns}
\end{frame}

\begin{frame}{Burnback Analysis \& Optimization}
  \textbf{Goal:} Optimize $A_b(t=0)$ to provide the $A_b(t)$ that enables the target thrust profile.
    \begin{itemize}
        \item \textbf{Analytical Methods:} Simplified geometries (cylinders, stars) allow closed-form $A_b(t)$ expressions.
        \item \textbf{Level Set Methods:}: Implicit surface tracking via signed distance functions.
        $$\phi(x,y,t) = \begin{cases}
            >0 & \text{in solid} \\
            =0 & \text{on surface} \\
            <0 & \text{in gas}
        \end{cases}$$
        $$\frac{\partial \phi}{\partial t} + r_b |\nabla \phi| = 0$$
        \item \textbf{Optimization:} Iterate on initial geometry to achieve desired $A_b(t)$ and thrust profile.
    \end{itemize}

\end{frame}

%==========================================================
% --- Sub-Section: Flow Physics ---
\subsection{Flow Physics \& Erosion}

\begin{frame}{Erosive Burning \& Velocity Coupling}
  \textbf{Phenomenon:} 
  \begin{itemize}
    \item Stagnant chamber assumption fails in high $L/D$ motors.
    \item High velocity ($u$) enhances heat transfer to the propellant, increasing burn rate beyond Saint-Robert's prediction.
  \end{itemize}
  
  \begin{block}{Lenoir-Robillard Model}
      $$r_{total} = r_{base} + r_{erosive}$$
      $$r_{base} = a P_c^n$$
      $$r_{erosive} = \alpha G^{0.8} L^{-0.2} e^{-\beta r_{total} \rho_p / G}$$
      \centering
      where $G = \rho_g u$ is the mass flux; $\alpha, \beta$ are empirical constants.
  \end{block}
  

\end{frame}

\begin{frame}{Implications of Erosive Burning}
  \begin{block}{System Implications:}
    Erosive burning typically occurs in high $L/D$ motors immediately after ignition when $u$ is highest. 
    As the grain burns, the port area ($A_{port}$) opens, reducing $u$ and thus erosive effects.
  \end{block}
  \begin{block}{Negative Erosion:}
    In some geometries, high velocity can cool the surface and slow the burning rate.
  \end{block}
\end{frame}

%==========================================================
% --- Sub-Section: Instability ---
\subsection{Combustion Instability}

\begin{frame}{Acoustic Instability: Hart-McClure Criterion}
  \begin{alertblock}{Combustion Instability}
    Acoustic eigenmodes in the chamber can couple with unsteady heat release from the burning propellant, 
    leading to large pressure oscillations or DC pressure increases that can damage or destroy the motor.
  \end{alertblock}

    \textbf{Stability:} Determined by the net growth rate $\alpha_{net}$.
    $$\alpha_{net} = \underbrace{\alpha_{pressure} + \alpha_{velocity}}_{\text{Gains (Driving)}} - \underbrace{(\alpha_{nozzle} + \alpha_{particle} + \alpha_{wall})}_{\text{Losses (Damping)}} $$
    $$\alpha_{net} < 0 \Rightarrow \text{Stable}; \quad \alpha_{net} > 0 \Rightarrow \text{Unstable}$$

    \begin{itemize}
        \item \textbf{Pressure Coupling:} Increased surface pressures can accelerate burning.
        \item \textbf{Particle Damping:} $Al_2O_3$ droplets lag gas oscil. \& dissipates energy via viscous drag.
        \item \textbf{Design Trade-off:} "Smokeless" missiles (no Al) lose particle damping (stability risk).
    \end{itemize}

\end{frame}




% --- Conclusion ---
\subsection{Summary}
\begin{frame}{Solid Propulsion Summary}
    \begin{block}{Takeaways}
    \begin{enumerate}
        \item \textbf{Chemistry is Destiny:} Chemistry dictates energy density and fundamental burn rate.
        \item \textbf{Geometry is Control:} Grain geometry determines the thrust profile ($P \propto K_n^{1/(1-n)}$).
        \item \textbf{Stability is Critical:} Acoustic eigenmodes must be damped to prevent failure.
        \item \textbf{Materials Limit Performance:} Nozzle erosion imposes limits on pressure and burn time.
    \end{enumerate}
    \end{block}
\end{frame}

\begin{frame}{Solid vs. Liquid Chemical Propulsion}
    \centering
    \begingroup
    \renewcommand{\arraystretch}{1.2}
    \rowcolors{2}{gray!10}{white}
    \resizebox{\textwidth}{!}{%
      \begin{tabular}{l c c}
        \toprule
        \textbf{Aspect} & \textbf{Solid Propulsion} & \textbf{Liquid Propulsion} \\
        \midrule
        Energy Density & High (volumetric) & Moderate (mass-based) \\
        Thrust Control & Limited (grain geometry) & Precise (throttleable) \\
        Ignition & Instantaneous & Requires ignition system \\
        Storability & Long-term stable & Cryogenic or limited life \\
        Complexity & Simple, few moving parts & Complex (pumps/valves) \\
        Reliability & High (few failure modes) & Moderate (more components) \\
        Applications & Boosters, missiles & Upper stages, deep space \\
        \bottomrule
      \end{tabular}%
    }
    \endgroup
\end{frame}

% --- Slide 1: The Governing Equation ---
\begin{frame}{Exhaust Velocity \& Specific Impulse: Solid vs. Liquid}

  \begin{block}{Ideal Exhaust Velocity \& Specific Impulse}
    $$ V_e = \sqrt{\frac{2 \gamma R_u}{\gamma - 1} \left( \frac{\mathbf{T_c}}{\mathbf{\mathfrak{M}}} \right) \left[ 1 - \left( \frac{P_e}{P_c} \right)^{\frac{\gamma-1}{\gamma}} \right]}$$
    $$ I_{sp} = \frac{V_e}{g_0} \propto \sqrt{\frac{T_c}{\mathfrak{M}}}$$
    \centering
    where $\gamma$ = specific heat ratio, $R_u$ = universal gas constant, $T_c$ = chamber temperature, \\ $\mathfrak{M}$ = molecular weight of exhaust, $P_c$ = chamber pressure, $P_e$ = exit pressure.
  \end{block}
    
  \vspace{1em}

  \textbf{$I_{sp}$ Drivers:}
  \begin{itemize}
      \item \textbf{Chamber Temperature ($T_c$):} Both solids and liquids achieve $3000K+$.
      \item \textbf{Molecular Weight ($\mathfrak{M}$):} \alert{$\mathfrak{M}_{liquid} < \mathfrak{M}_{solid}$} Lower $\mathfrak{M}$ yields higher velocity.
  \end{itemize}

\end{frame}

% --- Slide 2: Molecular Weight Comparison ---
\begin{frame}{Exhaust Molecular Weight ($\mathfrak{M}$) Differerences}
    \framesubtitle{\textit{Solid propellants yield heavy metal oxides and $Cl$ species; liquid $H_2$ and $CH_4$ yield light $H_2O$ and $CO_2$.}}
    \begin{block}{$H_2O + CO_2$ versus Metal Oxides + $Cl$}
        Liquid $H_2$ (and $CH_4$) yield low molecular weight exhaust streams, while solids are burdened by heavy metal oxides and chlorine species.
    \end{block}
    \vspace{2em}
    
    \centering
    \begingroup
    \renewcommand{\arraystretch}{1.2}
    %\rowcolors{2}{gray!10}{white}
    \begin{tabular}{l c c}
      	\toprule
      	\textbf{Property} & \textbf{LOX/LH2} & \textbf{AP/Al/HTPB} \\
      \midrule
      Products & $H_2O + H_2$ & $CO_2, HCl, Al_2O_3$ \\
      & (Fuel Rich) & \\
      Avg. $\mathfrak{M}$ & 9--12 g/mol & 28--32 g/mol \\
      $I_{sp}$ & $\approx 450$ s & $\approx 265$ s \\
      \bottomrule
    \end{tabular}
    \endgroup

\end{frame}

% --- Slide 3: Two-Phase Flow Losses ---
\begin{frame}{Two Phase Flow Loss Differences}
    \framesubtitle{\textit{Solid propellants use $Al$, yielding condensed-phase $Al_2O_3$ particles}}

    \begin{block}{Two Phase Flow Loss Drivers \& Implication}
        \begin{itemize}
            \item $Al_2O_3$ particles (liquid/solid) do not expand to do pressure work.
            \item \textbf{Velocity Lag:} Particles accelerate slower than gas.
            \item \textbf{Thermal Lag:} Particles stay hot, failing to transfer heat to kinetic energy.
            \item \textbf{Result:} 1--3\% $I_{sp}$ penalty.
        \end{itemize}
    
    \end{block}

    \centering
    % TikZ Nozzle Diagram
    \begin{tikzpicture}[scale=0.9]
        % Nozzle Shape
        \draw[thick, black] (-2,1.5) to[out=-20, in=180] (0,0.5) to[out=0, in=160] (3,1.8);
        \draw[thick, black] (-2,-1.5) to[out=20, in=180] (0,-0.5) to[out=0, in=-160] (3,-1.8);
        
        % Gas Streamlines (Fast)
        \draw[blue, ->, thick] (-1.5, 0.8).. controls (0, 0.2).. (2.8, 1.2) node[right] {Gas ($V_{gas}$)};
        \draw[blue, ->, thick] (-1.5, -0.8).. controls (0, -0.2).. (2.8, -1.2);
        
        % Particle Trajectories (Lagging)
        \foreach \x in {-1, 0, 1, 2}
            \fill[gray] (\x, 0) circle (2pt);
        
        \draw[gray, dashed, ->, thick] (-1.5, 0) -- (2.5, 0) node[right, black] {Particle ($V_p < V_{gas}$)};
        
        % Lag annotation
        \draw[red, <->] (2.0, 0) -- (2.0, 0.8);
        \node[red, right] at (2.0, 0.4) {Slip Velocity};
    \end{tikzpicture}

\end{frame}

% --- Slide 4: The Density Trade-off ---
\begin{frame}{The Trade-off: Density Impulse}
    \framesubtitle{\textit{Solids lose on Specific Impulse ($I_{sp}$) but win on Density Impulse ($I_{sp} \times \rho$).}}
    
    \begin{columns}
        \column{0.6\textwidth}
        \textbf{Strategic Implications:}
        \begin{itemize}
            \item \textbf{Liquid Hydrogen:} $\rho \approx 0.07$ g/cm$^3$. Requires massive tank volume. High drag, high structural mass.
            \item \textbf{Solid Propellant:} $\rho \approx 1.75$ g/cm$^3$. Compact storage.
            \item \textbf{Application:} Solids are ideal for first stages (Boosters) where thrust-to-volume is critical. Liquids are ideal for upper stages where mass efficiency ($I_{sp}$) dominates.
        \end{itemize}

        \column{0.4\textwidth}
        \centering
        % TikZ Volume Comparison
        \begin{tikzpicture}[scale=0.7]
            % Solid Motor
            \fill[gray!50] (0,0) rectangle (1.5, 4);
            \node at (0.75, 2) {\textbf{Solid}};
            \node[below] at (0.75, 0) {High $\rho$};
            
            % Liquid Tank
            \draw[blue, thick] (3,0) rectangle (5.5, 6);
            \fill[blue!10] (3,0) rectangle (5.5, 6);
            \node at (4.25, 3) {\textbf{LH2 Tank}};
            \node[below] at (4.25, 0) {Low $\rho$};
            
            \node[align=center] at (2.75, -1.5) {For Equal Total Impulse};
        \end{tikzpicture}
    \end{columns}
\end{frame}

% --- Slide 5: Summary ---
\begin{frame}{Solid vs Liquid Comparision Summary}
    \framesubtitle{\textit{Liquids achieve higher $I_{sp}$ due to lightweight fuels, while solids suffer from heavier fuels and two-phase flow losses.}}

    \begin{table}
        \centering
        \begingroup
        \renewcommand{\arraystretch}{1.2}
        \rowcolors{2}{gray!10}{white}
        \begin{tabular}{l c c}
          	\toprule
          	\textbf{Parameter} & \textbf{Solids (APCP)} & \textbf{Liquids (LOX/LH2)} \\
          \midrule
          	\textbf{Combustion Temp ($T_c$)} & $\approx 3400$ K & $\approx 3600$ K \\
          	\textbf{Exhaust MW ($\mathfrak{M}$)} & \alert{High ($\sim30$)} & \alert{Low ($\sim12$)} \\
          	\textbf{Exhaust Phase} & Gas + Solids & Pure Gas \\
          	\textbf{Loss Mechanisms} & Two-Phase Lag & Dissociation \\
          	\textbf{Typical $I_{sp}$ (Vac)} & $\sim 280$ s & $\sim 450$ s \\
          \bottomrule
        \end{tabular}
        \endgroup
    \end{table}
    
    \end{frame}

% ======================================================================================================================    
\section{Liquid Monopropellant Propulsion}

\begin{frame}{Lecture Outline}
  \tableofcontents[currentsection]
\end{frame}

\begin{frame}{Liquid Monopropellant Thrusters}
    \framesubtitle{\textit{Leverages a single propellant that exothermically decomposes over a catalyst}}

    \begin{block}{Monopropellant thrusters:}
        \begin{itemize}
        \item Use a \textbf{single fluid} (e.g., $N_2H_4$ [hydrazine] or $H_2O_2$ [peroxide])
        \item Decompose the fluid \textbf{catalytically} to form a hot gas mixture
        \item Expands mixture through a \textbf{C--D nozzle} to generate thrust
        \item Avoids mixture ratio control, igniters, and complex plumbing
        \item Used for attitude control, station keeping, "small" $\Delta v$ maneuvers
        \end{itemize}
    \end{block}

    \vspace{2em}

    % Monopropellant thrust schematics
    \centering
    \begin{tikzpicture}[node distance=1.0cm, >=latex]

        \tikzset{
            block/.style={draw, thick, minimum width=1.8cm, minimum height=1cm, align=center, fill=blue!20},
            smallblock/.style={draw, thick, minimum width=1.3cm, minimum height=0.8cm, align=center, fill=green!20}
        }

        \node[block, fill=cyan!30] (tank) {Propellant\\Tank};
        \node[smallblock, fill=orange!30, right=0.8cm of tank] (valve) {Valve};
        \node[block, fill=purple!30, right=0.8cm of valve] (cat) {Catalyst\\Bed};
        \node[block, fill=yellow!40, right=0.8cm of cat] (chamber) {Decomp.\\Chamber};
        \node[block, fill=red!30, right=0.8cm of chamber] (nozzle) {Nozzle};

        \draw[->, thick] (tank) -- (valve);
        \draw[->, thick] (valve) -- (cat);
        \draw[->, thick] (cat) -- (chamber);
        \draw[->, thick] (chamber) -- (nozzle);

    \end{tikzpicture}

\end{frame}

\begin{frame}{Monopropellant Comparison}
    \framesubtitle{\textit{Hydrazine vs High Test Hydrogen Peroxide}}
    \centering
    \small
    \renewcommand{\arraystretch}{1.2}
    \rowcolors{2}{gray!10}{white}
    \begin{tabular}{l l l}
        \toprule
        Aspect & Hydrazine (N$_2$H$_4$) & High-Test Peroxide (H$_2$O$_2$) \\
        \midrule
        Primary Reaction & $3\,\text{N}_2\text{H}_4 \rightarrow 4\,\text{NH}_3 + \text{N}_2$ & $2\,\text{H}_2\text{O}_2 \rightarrow 2\,\text{H}_2\text{O} + \text{O}_2 + \text{heat}$ \\
        Secondary Step & $4\,\text{NH}_3 \rightarrow 2\,\text{N}_2 + 6\,\text{H}_2$ (cracking) & -- \\
        Catalyst & Ir/Al$_2$O$_3$ bed & Silver or Pt mesh \\
        Products & $N_2$, $H_2$, $NH_3$ (some uncracked NH$_3$) & Steam ($H_2O$) + $O_2$ \\
        Nominal $T_c$ & $\sim 1000\,$K & $850$--$1150\,$K (conc. dependent) \\
        Typical $\gamma$ & $\approx 1.30$ & $1.25$--$1.30$ \\
        Gas Constant $R$ & $\sim 400\,$J/kg-K & $300$--$350\,$J/kg-K \\
        Vacuum $I_{sp}$ & $220$--$235\,$s & $150$--$180\,$s \\
        \bottomrule
    \end{tabular}

    \vspace{0.5em}

\end{frame}


% ======================================================================================================================    
\section{Electric Propulsion}

%---------------------------------------------------------
% Section 1: Introduction & Physics
%---------------------------------------------------------

\begin{frame}{Lecture Outline}
  \tableofcontents[currentsection]
\end{frame}

\begin{frame}{Chemical to Electric Propulsion}
    \begin{block}{Chemical Propulsion:}
      \begin{itemize}
          \item Energy source -- reacting fuel and oxidizer -- is the propellant.
          \item Max $v_e \approx 4.5$ km/s ($H_2/O_2$).
      \end{itemize}
    \end{block}
    \vspace{0.5cm}
    \begin{block}{Electric Propulsion:}
      \begin{itemize}
          \item Energy source (e.g., solar, nuclear) decoupled from propellant.
          \item $v_e > 30$ km/s  achievable.
          \item At fixed power, $I_{sp}$ and thrust are inversely related.
      \end{itemize}
    \end{block}
\end{frame}

\begin{frame}{Power-Limited Rocket Equation}
  \framesubtitle{\textit{Thrust ($T$) is \textit{inversely proportional} to Specific Impulse ($I_{sp}$) at fixed power ($P_{in}$)}}

    Thrust ($T$) is the momentum flux of the beam:
    \begin{equation}
        T = \dot{m} v_e
    \end{equation}

    Jet Power ($P_{jet}$) is the kinetic energy flux of the beam:
    \begin{equation}
        P_{jet} = \frac{1}{2} \dot{m} v_e^2 = \frac{1}{2} T v_e
    \end{equation}
    
    Overall efficiency is given by $\eta_T = P_{jet} / P_{in}$:
    \begin{equation}
        T = \frac{2 \eta_T P_{in}}{v_e} = \frac{2 \eta_T P_{in}}{g_0 I_{sp}}
    \end{equation}
    
    \centering
    \alert{\textbf{Design Trade:} Minimize propellant mass (high $I_{sp}$) vs. Minimize trip time (high Thrust).}

\end{frame}

\begin{frame}{Efficiency Drivers}
  \framesubtitle{\textit{Electrical conversion, ionization fraction, beam divergence, voltage utilization, doubles correction}}
  
  \begin{block}{Thruster overall efficiency ($\eta_T$)}
    $$\eta_T = \eta_e \cdot \eta_m \cdot \eta_b \cdot \eta_v \cdot \alpha$$
  \end{block}
  
  \vspace{2em}
    
  \begin{itemize}
      \item \textbf{Electrical Efficiency ($\eta_e$):} Beam / Input Electric power (magnets, heat, PPU losses). 
      \item \textbf{Mass Utilization ($\eta_m$):} Fraction of propellant ionized ($\dot{m}_{ion} / \dot{m}_{total}$).
      \item \textbf{Beam Divergence ($\eta_b$ or $\eta_{div}$):} Loss due to plume spread ($\langle \cos \theta \rangle^2$).
      \item \textbf{Voltage Utilization ($\eta_v$):} Effective beam voltage vs. Discharge voltage ($V_b / V_d$).
      \item \textbf{Doubles Correction ($\alpha$):} Thrust loss due to multiply charged ions ($Xe^{++}$).
  \end{itemize}

\end{frame}

%---------------------------------------------------------
% Comparative Table & Design Guide
%---------------------------------------------------------
\begin{frame}{Comparative Analysis of Electric Propulsion Systems}
    \centering
    \begingroup
    \renewcommand{\arraystretch}{1.2}
    \rowcolors{2}{gray!10}{white}
    \resizebox{\textwidth}{!}{%
      \begin{tabular}{l c c c c c p{3.5cm} p{3.5cm}}
        \toprule
        \textbf{Technology} & \textbf{$I_{sp}$ (s)} & \textbf{Thrust} & \textbf{$\eta_T$} & \textbf{Power} & \textbf{TRL} & \textbf{Key Pros} & \textbf{Key Cons} \\
        \midrule
        \textbf{Resistojet} & 280--350 & 10mN--0.5N & 65--80\% & $<1$ kW & 9 & Simple; Low cost; Shared propellant & Material thermal limits; Low $I_{sp}$ \\
        \midrule
        \textbf{Arcjet} & 450--600 & 0.1N--2N & 30--45\% & 0.5--2 kW & 9 & Robust; Higher $I_{sp}$ than resistojet & Electrode erosion; Frozen flow losses \\
        \midrule
        \textbf{Gridded Ion} & 2500--4000+ & 20--250mN & 60--80\% & 0.5--7 kW & 9 & Highest Efficiency; Longest Life & Low thrust density (Grid limits); Complex PPU \\
        \midrule
        \textbf{Hall Effect} & 1500--3000 & 40mN--1N+ & 50--65\% & 0.2--20 kW & 9 & High Thrust-to-Power; Compact & Beam divergence; Channel erosion (if unshielded) \\
        \midrule
        \textbf{Electrospray} & 1000--6000 & $\mu$N--mN & $>70\%$ & mW--50W & 6--9 & Precision control; Scalable arrays & Very low thrust; High voltage; Clogging risks \\
        \midrule
        \textbf{PPT} & 800--1500 & $\mu$N-s & 5--15\% & $<100$ W & 9 & Solid fuel (Teflon); Simple storage & Very low efficiency; Carbon contamination \\
        \midrule
        \textbf{MPD} & 2000--10000 & 1N--100N & 30--60\% & MW Class & 4--5 & High thrust density at high $I_{sp}$ & Requires MW power; Cathode erosion severe \\
        \bottomrule
      \end{tabular}%
    }
    \endgroup
\end{frame}

\begin{frame}{Designer's Guide to Thruster Selection (1 of 2)}

    \textbf{1. Is Power the Primary Constraint?}
    \begin{itemize}
        \item \textbf{Selection:} \textcolor{blue}{Hall Effect Thruster}
        \item \textit{Rationale:} Offers higher Thrust-to-Power ratio (60--80 mN/kW) than Ion engines. Reduces maneuver time for power-limited commercial satellites.
    \end{itemize}
    \vspace{0.2cm}
    
    \textbf{2. Is Propellant Mass the Primary Constraint?}
    \begin{itemize}
        \item \textbf{Selection:} \textcolor{blue}{Gridded Ion Thruster}
        \item \textit{Rationale:} Maximizes Specific Impulse ($I_{sp} > 3000$ s). Essential for high-$\Delta v$ deep space missions (e.g., Dawn, BepiColombo) to minimize launch mass.
    \end{itemize}
    \vspace{0.2cm}
    
\end{frame}

\begin{frame}{Designer's Guide to Thruster Selection (2 of 2)}
    
  \textbf{3. Is Cost or Volume Constrained?}
    \begin{itemize}
        \item \textbf{Selection:} \textcolor{blue}{Argon Hall} (Cost) or \textcolor{blue}{Iodine Hall} (Volume)
        \item \textit{Rationale:} Argon is abundant and cheap for constellations (Starlink). Iodine stores as a dense solid, eliminating high-pressure tanks for CubeSats.
    \end{itemize}
    \vspace{0.2cm}
    
    \textbf{4. Is Precision Pointing Required?}
    \begin{itemize}
        \item \textbf{Selection:} \textcolor{blue}{FEEP / Electrospray}
        \item \textit{Rationale:} Provides $\mu$N thrust resolution with no moving parts (valves).
    \end{itemize}

\end{frame}

%---------------------------------------------------------
% Electrothermal Systems: Resistojets & Arcjets
%---------------------------------------------------------
\subsection{Electrothermal}

\begin{frame}{Electrothermal: Resistojets \& Arcjets}
  \framesubtitle{\textit{Propellant energization \& acceleration via electric heating.}}

    \textbf{Resistojets:}
    \begin{itemize}
        \item Mechanism: Gas flows over electrically heated element (Re, W).
        \item Limit: Material melting point.
        \item Performance: $I_{sp} \approx 300$ s (Hydrazine).
        \item Application: Station-keeping, attitude control.
    \end{itemize}

    \vspace{2em}
    
    \textbf{Arcjets:}
    \begin{itemize}
        \item Mechanism: High-current arc heats gas core $>10{,}000$ K.
        \item Physics: Cool boundary layer protects nozzle walls.
        \item Performance: $I_{sp} \approx 500-600$ s.
        \item Limitation: Electrode erosion, high power density required.
    \end{itemize}

\end{frame}

\begin{frame}{Electrothermal: Arcjet Thruster}
  \begin{figure}[h]
  \centering
  \begin{tikzpicture}
    % Nozzle / Anode (Cutaway)
    \draw[thick, fill=gray!20] (0,1) -- (2,1) -- (2.5,0.3) -- (4.5,0.8) -- (4.5,1.2) -- (0,1.2) -- cycle;
    \draw[thick, fill=gray!20] (0,-1) -- (2,-1) -- (2.5,-0.3) -- (4.5,-0.8) -- (4.5,-1.2) -- (0,-1.2) -- cycle;
    
    % Cathode Rod
    \draw[thick, fill=black!80] (0,0.15) -- (1.8,0.1) -- (1.8,-0.1) -- (0,-0.15) -- cycle;
    
    % Arc
    \draw[thick, orange, decorate, decoration={zigzag, segment length=2mm, amplitude=1mm}] (1.8,0) -- (2.4,0);
    
    % Propellant Flow
    \draw[->, blue, thick] (0.5, 0.6) -- (1.5, 0.6);
    \draw[->, blue, thick] (0.5, -0.6) -- (1.5, -0.6);
    
    % Plume
    \shade[left color=orange!50, right color=white, opacity=0.5] (2.5,0.3) -- (4.5, 0.8) -- (4.5, -0.8) -- (2.5,-0.3) -- cycle;
    
    % Labels - positioned outside/around the figure
    \node at (-1.2, 0) {\textbf{Cathode (-)}};
    \node at (2.5, 1.8) {\textbf{Anode (+) / Nozzle}};
    \draw[<-] (2.1, 0) -- (2.1, -1.8) node[below] {\small Constrictor / Arc};
    \node at (0.3, 0.35) [blue, above left] {\small Propellant};
    \node at (5.8, 0) {\small Exhaust};

  \end{tikzpicture}
  \caption{Arcjet Thruster Schematic}
  \end{figure}
\end{frame}

%---------------------------------------------------------
% Electrostatic Systems: Gridded Ion & Hall Effect Thrusters
%---------------------------------------------------------
\subsection{Electrostatic}

\begin{frame}{Electrostatic: Gridded Ion Thruster (GIT)}
  \framesubtitle{\textit{\textbf{Ionized} propellant acceleration via electric fields}}

    \begin{columns}
        \column{0.5\textwidth}
        \textbf{Core Concept:}
        \begin{itemize}
            \item Decoupled ionization and acceleration.
            \item Highest efficiency electric propulsion device.
        \end{itemize}
        \vspace{0.2cm}
        \textbf{Ionization Generation:}
        \begin{itemize}
            \item \textbf{DC:} Hollow cathode + Ring-cusp magnetic confinement.
            \item \textbf{RF:} Inductive coil, electrodeless discharge (longer life).
        \end{itemize}
        
        \column{0.5\textwidth}
        \textbf{Acceleration:}
        \begin{itemize}
            \item Multi-grid assembly (Screen + Accel).
            \item Electrostatic field extracts ions.
        \end{itemize}
    \end{columns}
\end{frame}

\begin{frame}{Electrostatic: GIT Schematic}

  \begin{figure}[h]
    \centering
    \begin{tikzpicture}
        % Discharge Chamber
        \draw[thick] (0,0) rectangle (5, 4);
        
        % Hollow Cathode
        \draw[thick, fill=gray!30] (0, 1.8) rectangle (1, 2.2);
        \node at (0.5, 2) {\tiny Cathode};
        
        % Magnetic Rings (Cusps)
        \foreach \x in {1, 2.5, 4} {
            \draw[fill=red] (\x, 4) circle (0.15);
            \draw[fill=red] (\x, 0) circle (0.15);
        }
        \node at (2.5, 4.5) {Ring Magnets};
        
        % Grids
        \draw[thick, dashed] (5.2, 0) -- (5.2, 4);
        \draw[thick, dashed] (5.5, 0) -- (5.5, 4);
        \node at (5.35, 4.5) {Ion Optics};
        \draw[<-] (5.2, 3.5) -- (3.5, 5) node[above] {Screen Grid (+V)};
        \draw[<-] (5.5, 3.5) -- (7.0, 5) node[above] {Accel Grid (-V)};
        
        % Neutralizer
        \draw[thick, fill=gray!30] (6, 0) rectangle (7, 0.5);
        \node at (6.5, -0.3) {Neutralizer};
        \draw[->, dashed] (7, 0.25) -- (7.5, 1);
        
        % Ions
        \foreach \y in {1, 2, 3} {
            \draw[->, blue, thick] (4.5, \y) -- (7, \y);
            \fill[blue] (4.5, \y) circle (0.05);
            \node at (4.3, \y) {\tiny +};
        }
        \node at (8.0, 2) {Ion Beam};
        \node at (2.5, 2) {Discharge Plasma};

    \end{tikzpicture}
    \caption{Electrostatic: Gridded Ion Thruster}

  \end{figure}

\end{frame}

\begin{frame}{Child-Langmuir Limit}
    \framesubtitle{\textit{Current/Thrust density is limited by space charge shielding between grid: Child--Langmuir Law}}
    
    \begin{block}{Child--Langmuir Law for Ion Thrusters:}
        $$J_{CL} = \frac{4 \epsilon_0}{9} \sqrt{\frac{2q}{m_i}} \frac{V_{gap}^{3/2}}{d^2}$$
    \end{block}
    \vspace{1em}
    
    Where:
    \begin{itemize}
        \item $J = $ Current density ($A/m^2$)
        \item $V_{gap} = $ Screen gap voltage ($V_{screen} - V_{accel}$)
        \item $d = $ Grid gap
    \end{itemize}
    \vspace{1em}
    
    \begin{alert}{Implications:} 
      \begin{itemize}
        \item To increase $J$ (and thrust density), increase voltage and decrease grid spacing.
        \item Heavier ions (larger $m_i$) reduce $J_{CL}$.
        \item Screen--accelerator gap behaves like a planar diode.
      \end{itemize}
    \end{alert}
\end{frame}

\begin{frame}{Child-Langmuir Law Derivation (1 of 2)}
  \framesubtitle{\textit{1D Planar diode with voltage $V$ across gap $d$.  Ions of mass $m$ and charge $q$ starting from rest at $x=0$.}}
  \small
  \begin{itemize}
    \item Poisson's equation for the electric potential $\phi$ ($\vec{E} = -\nabla \phi$):
    $$
    \frac{d^2\phi}{dx^2} = -\frac{\rho}{\varepsilon_0}
    = -\frac{q n}{\varepsilon_0}
    $$
    {\centering\small$q$ = ion charge, $n$ = ion density, $\epsilon_{0}$ = permittivity of free space.\par}

    \item Continuity and energy conservation equations:
    $$
    J = q n v = \text{const}, \qquad
    \frac{1}{2} m v^2 = q\phi
    $$
    \item Solve continuity and energy conservation equations for the charge density $n$:
    $$
    n = \frac{J}{q v} = \frac{J}{q}
      \sqrt{\frac{m}{2q}}\,\phi^{-1/2}
    $$
    \item Substitute the expresssion for $n$ into Poisson's equation:
    $$
    \frac{d^2\phi}{dx^2}
    = \frac{J}{\varepsilon_0}\sqrt{\frac{m}{2q}}\,\phi^{-1/2}
    $$
  \end{itemize}
\end{frame}


\begin{frame}{Child-Langmuir Law Derivation (2 of 2)}
  \framesubtitle{\textit{Manipulate \& Integrate (twice) to yield $J_{CL}$ expression mentioned earlier.}}
  \tiny
  \begin{itemize}
    \item Let $y = d\phi/dx$:
    $$ \frac{d^2\phi}{dx^2} = \frac{d y}{dx} = \frac{dy}{d\phi}
      \frac{d\phi}{dx} = y \frac{dy}{d\phi} $$
    \item So Poisson's equation becomes:
    $$
    y \frac{dy}{d\phi}
    = \frac{J}{\varepsilon_0}\sqrt{\frac{m}{2q}}\,\phi^{-1/2}
    $$
    \item Integrate with $\phi(0)=0$ at the emitting surface:
    $$
    \frac{1}{2}y^2 
      = \frac{2J}{\varepsilon_0}\sqrt{\frac{m}{2q}}\,\phi^{1/2}
    $$
    \item So
    $$
    \frac{d\phi}{dx} = C\,\phi^{1/4}, \quad
    C=\left(\frac{4J}{\varepsilon_0}\sqrt{\frac{m}{2q}}\right)^{1/2}
    $$
    \item Integrate from $0$ to $d$, $\phi: 0\to V$:
    $$
    \int_0^V \phi^{-1/4}d\phi = C\int_0^d dx
    \Rightarrow \frac{4}{3}V^{3/4} = C d
    $$
    \item Solve for $J$:
    $$
    J = \frac{4}{9}\,\varepsilon_0
      \sqrt{\frac{2q}{m}}\,
      \frac{V^{3/2}}{d^2}
    $$
  \end{itemize}
\end{frame}

\begin{frame}{Numerical Example: Xenon Gridded Ion Thruster}
  \small
  \begin{itemize}
    \item Xe$^+$, $q=e$, $m \approx 2.18\times 10^{-25}\,\mathrm{kg}$.
    \item Grid spacing: $d = 1 mm$.
    \item Beam voltage: $V = 1100 V$.
    \item Child--Langmuir current density:
    $$
    J_{\mathrm{CL}} \approx 1.7 \times 10^{2} A/m^2
    \approx 0.17 A/cm^2
    $$
    \item For a $15 cm$ diameter active area:
    $$
    A \approx 1.8 \times 10^{-2} m^2, \quad
    I \approx J A \approx 3 A
    $$
    \item Thrust:
    $$
    T = I \sqrt{\frac{2 m V}{q}} \approx 0.17 N
    $$
    \item Very close to real NEXT / NSTAR thruster operating points.
  \end{itemize}
\end{frame}

\iffalse
\begin{frame}{Design Implications for Ion Thrusters}
  \begin{itemize}
    \item \textbf{Maximum thrust density}:
      \begin{itemize}
        \item $J \le J_{\mathrm{CL}}$ limits $I_{\mathrm{beam}}$ and thus thrust.
      \end{itemize}
    \item \textbf{Grid optics and lifetime}:
      \begin{itemize}
        \item Operating above $J_{\mathrm{CL}}$ distorts equipotentials.
        \item Increases ion impingement on accelerator grid $\rightarrow$ erosion.
      \end{itemize}
    \item \textbf{Electron backstreaming}:
      \begin{itemize}
        \item Space-charge effects can reduce the downstream potential barrier.
        \item Electrons can backstream into the discharge if the accelerator voltage is too low.
      \end{itemize}
    \item In practice:
      \begin{itemize}
        \item Thrusters are designed to operate somewhat \emph{below} $J_{\mathrm{CL}}$.
        \item Trade-offs among thrust density, efficiency, and grid life.
      \end{itemize}
  \end{itemize}
\end{frame}
%------------------------------------------------

\begin{frame}{GIT Summary}
  \begin{itemize}
    \item Child--Langmuir law gives the \textbf{space-charge-limited} ion current density in the screen--accelerator gap.
    \item For a given ion species, gap $d$ and voltage $V$, $J_{\mathrm{CL}}$ is fixed.
    \item In gridded ion thrusters this:
      \begin{itemize}
        \item Caps \textbf{thrust per unit area}.
        \item Constrains \textbf{grid spacing} and \textbf{voltage} choices.
        \item Interacts with \textbf{beam optics}, \textbf{erosion}, and \textbf{backstreaming}.
      \end{itemize}
    \item Space-charge physics is therefore central to ion thruster design and scaling.
  \end{itemize}
\end{frame}
%------------------------------------------------

\begin{frame}{Optics Lifetime: Erosion Physics}
    \textbf{Primary Failure Mode: Accelerator Grid Erosion}
    \begin{itemize}
        \item Mechanism: Charge Exchange (CEX) Collisions.
        \item Fast Ion + Slow Neutral $\rightarrow$ Fast Neutral + Slow Ion.
        \item Slow ions form in the gap/plume and accelerate into the negative Accel Grid.
        \item Result: "Pits and Grooves" erosion.
    \end{itemize}
    \vspace{0.5cm}
    \textbf{Perveance Matching:}
    \begin{itemize}
        \item \textbf{Over-perveance:} Sheath bulges, direct impingement.
        \item \textbf{Under-perveance:} Beam focuses too sharply, CEX crossover.
    \end{itemize}
\end{frame}
\fi

%---------------------------------------------------------
% Electrostatic Hall Effect Thrusters (HET)
%---------------------------------------------------------

\begin{frame}{Electrostatic: Hall Thruster Operating Principle}
  \framesubtitle{\textit{Ionizing magnetic electrons trapped by radial $\vec{B}$ field.  Non-magnetic (e.g., $Xe^+$) ions accelerated by axial $\vec{E}$ field.}}
    \textbf{Cross-Field ($E \times B$) Discharge:}
    \begin{itemize}
        \item Radial Magnetic Field ($B_r$) + Axial Electric Field ($E_z$).
        \item Light electrons are magnetized. Heavy ions are un-magnetized.
        \item Electrons undergo an azimuthal $\mathbf{E}\times\mathbf{B}$ Hall drift.
        \item Ions accelerated axially by $E_z$ and exit at $\sim\!10$--$20~\mathrm{km/s}$.
        \item Quasi-neutral plasma throughout; no grids $\Rightarrow$ No space charge limit!
    \end{itemize}
    
    \begin{alertblock}{Magnetic Insulation:} 
      \begin{itemize}
        \item Electrons are trapped by $\vec{B}$ field, limiting axial current.
        \item High discharge voltage ($V_d \approx 300$--$600$ V) achievable in quasi-neutral plasma.
        \item High $V_d$ + No Space Charge Limit $\Rightarrow$ Higher thrust density than Ion Thrusters.
      \end{itemize}
    \end{alertblock}

\end{frame}

\iffalse
\begin{frame}{Electrostatic: Hall Effect Thuster Schematic}

  \begin{figure}[h]
    \centering
    \begin{tikzpicture}[yscale=0.65]
        % Central Core (Inner Pole)
        \draw[thick, fill=gray!40] (0, -1) rectangle (3, 1);
        \node at (1.5, 0) {Inner Magnetic Pole};
        
        % Inner Coil
        \draw[pattern=north east lines] (0.5, 0.2) rectangle (2.5, 0.8);
        \draw[pattern=north east lines] (0.5, -0.8) rectangle (2.5, -0.2);
        
        % Discharge Channel Walls
        \draw[thick] (3, 1.2) -- (0, 1.2); % Inner wall top
        \draw[thick] (3, 2.2) -- (0, 2.2); % Outer wall top
        
        \draw[thick] (3, -1.2) -- (0, -1.2); % Inner wall bottom
        \draw[thick] (3, -2.2) -- (0, -2.2); % Outer wall bottom
        
        % Outer Pole
        \draw[thick, fill=gray!40] (0, 2.2) -- (3, 2.2) -- (3, 2.8) -- (0, 2.8) -- cycle;
        \draw[thick, fill=gray!40] (0, -2.2) -- (3, -2.2) -- (3, -2.8) -- (0, -2.8) -- cycle;
        
        % Anode / Gas Distributor
        \draw[thick, fill=orange] (0.2, 1.3) rectangle (0.5, 2.1);
        \draw[thick, fill=orange] (0.2, -1.3) rectangle (0.5, -2.1);
        \node at (0, 1.7) [left] {Anode (+V)};
        
        % Magnetic Field Lines (Radial)
        \draw[blue, dashed, ->] (3, 1).. controls (3.2, 1.6).. (3, 2.2);
        \draw[blue, dashed, ->] (3, -1).. controls (3.2, -1.6).. (3, -2.2);
        \node at (4.5, 0) [blue] {Radial B-Field};
        
        % Cathode
        \draw[thick, fill=gray!20] (3.5, 3) rectangle (4.5, 3.5);
        \node at (4, 3.8) {Cathode};
        \draw[->, dashed] (4, 3) -- (3.5, 2.5) node[midway, right] {$e^-$};
        
        % Beam
        \shade[left color=purple!40, right color=white, opacity=0.5] (3, 1.2) -- (6, 2.5) -- (6, 0.9) -- (3, 2.2) -- cycle;
        \shade[left color=purple!40, right color=white, opacity=0.5] (3, -1.2) -- (6, -2.5) -- (6, -0.9) -- (3, -2.2) -- cycle;
        
        \node at (5, 1.7) {Ion Beam};

    \end{tikzpicture}
    \caption{Electrostatic: Hall Effect Thruster (Cross Section)}
  \end{figure}

\end{frame}
\fi

\begin{frame}{Electrostatic: Hall Effect Thruster Cross-Section}
  \begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{figs/Wfm_hall_thruster.pdf}
    \caption{Source: \href{https://en.wikipedia.org/wiki/Hall-effect_thruster}{Wikipedia}}
  \end{figure}
\end{frame}

\begin{frame}{Propellant Economics: The Shift from Xenon}
    \begin{table}
    \centering
  \begingroup
  \renewcommand{\arraystretch}{1.2}
  \rowcolors{2}{gray!10}{white}
  \begin{tabular}{l c c c}
    \toprule
    \textbf{Propellant} & \textbf{Mass (amu)} & \textbf{Cost} & \textbf{Use Case} \\
    \midrule
    \textbf{Xenon} & 131.3 & High ($\sim$\$3k/kg) & Science / GEO \\
    \textbf{Krypton} & 83.8 & Moderate & Starlink V1 \\
    \textbf{Argon} & 39.9 & Low ($\sim$\$10/kg) & Starlink V2 \\
    \textbf{Iodine} & 126.9 & Moderate & CubeSats (\alert{Solid!}) \\
    \bottomrule
  \end{tabular}
  \endgroup
    \end{table}
    \vspace{0.2cm}
    \textbf{Starlink V2 (Argon):}
    \begin{itemize}
        \item 4.2 kW, $I_{sp} \approx 2500$ s.
        \item $\eta_T \approx 50\%$ (Lower than Xe, but economically viable).
        \item High ionization energy of Ar (15.8 eV) challenges efficiency.
    \end{itemize}
\end{frame}

\iffalse
\begin{frame}{Electron Gyromotion and Azimuthal Drift in a HET}
  \centering
  \begin{tikzpicture}[>=Latex, scale=1.2]

    % Outer and inner channel radii
    \def\Rout{2.2}
    \def\Rin{1.4}

    % Draw channel walls (top view annulus)
    \draw[thick] (0,0) circle (\Rout);
    \draw[thick] (0,0) circle (\Rin);
    \node at (0,0) {\small Channel};

    % Azimuthal drift path (mid-radius)
    \def\Rm{1.8}
    \draw[very thick, dashed, gray] (0,0) circle (\Rm);
    \node[gray] at (0,-2.5) {\small Azimuthal $\mathbf{E}\times\mathbf{B}$ drift path};

    % Big arrow showing direction of azimuthal drift
    \draw[->, ultra thick, green!60!black] (\Rm,0) arc (0:60:\Rm);
    \node[green!60!black] at (1.7,1.3) {\small $\mathbf{v}_{E\times B}$};

    % Draw several small electron gyro-orbits along the drift path
    \foreach \angle in {30,100,180,260,320} {
      \path (\angle:\Rm) coordinate (C);
      \draw[blue, thick] (C) circle (0.15);
    }

    % Mark one electron on a gyro-orbit with a dot and arrow
    \path (30:\Rm) coordinate (C1);
    \draw[blue, thick] (C1) circle (0.15);
    \fill[blue] ($(C1)+(0.15,0)$) circle (0.04);
    \draw[->, blue, thick] ($(C1)+(0.15,0)$) arc (0:60:0.15);
    \node[blue] at (2.3,0.6) {\small Gyromotion};

    % Show that the gyro radius is small compared to channel width
    \draw[<->] (\Rin, -1.0) -- (\Rout, -1.0);
    \node at (0,-1.1) {\small Channel width};
    \draw[<->] (C1) ++(0.2,-0.7) -- ++(0.15,0);
    \node[right] at ($(C1)+(0.35,-0.7)$) {\small $r_{L,e} \ll$ channel size};

    % Axes and field directions (side legend)
    % E field (axial, out of page/top view)
    \draw[->, thick, red] (3.0,0.0) -- (3.0,1.0);
    \node[red, right] at (3.0,1.0) {$\mathbf{E}$ (axial)};

    % B field (radial, inward)
    \draw[->, thick, blue] (3.0,0.0) -- (2.0,0.0);
    \node[blue, below] at (2.0,0.0) {$\mathbf{B}$ (radial)};

    % E x B direction (azimuthal)
    \draw[->, thick, green!60!black] (3.0,0.0) -- (3.6,-0.6);
    \node[green!60!black, right] at (3.6,-0.6) {$\mathbf{E}\times\mathbf{B}$};

  \end{tikzpicture}
\end{frame}

\begin{frame}{Anomalous Transport \& Instabilities}
    \textbf{Anomalous Transport:}
    \begin{itemize}
        \item Classical collision theory predicts current $100\times$ lower than observed.
        \item Real transport driven by:
        \begin{itemize}
            \item Plasma turbulence/waves (Bohm diffusion).
            \item Near-Wall Conductivity (Secondary electron emission).
        \end{itemize}
    \end{itemize}
    \vspace{0.2cm}
    \textbf{Breathing Mode Instability (10-30 kHz):}
    \begin{itemize}
        \item "Predator-Prey" cycle between Neutrals and Ions.
        \item Ionization depletes neutrals $\rightarrow$ Current drops $\rightarrow$ Neutrals refill $\rightarrow$ Current spikes.
        \item Requires active filtering in PPU.
    \end{itemize}
\end{frame}

\begin{frame}{The Revolution: Magnetic Shielding}
    \textbf{The Problem (Pre-2010):}
    \begin{itemize}
        \item Field lines intersected ceramic walls. High energy ions eroded channel.
        \item Lifetime limit: $\approx 5,000$ hours.
    \end{itemize}
    \vspace{0.2cm}
    \textbf{The Solution (Magnetic Shielding):}
    \begin{itemize}
        \item Topology shaping: Field lines parallel to walls, grazing the anode.
        \item Creates equipotential zone near wall ($T_e$ is low).
        \item Potential drop < Sputtering threshold.
        \item \textbf{Result:} Erosion reduced $100-1000\times$. Lifetimes $>50{,}000$ hours (e.g., AEPS for Lunar Gateway).
    \end{itemize}
\end{frame}
\fi

%---------------------------------------------------------
% Section 5: Electromagnetic & Propellants
%---------------------------------------------------------
\subsection{Electromagnetic}

\begin{frame}{Electromagnetic Propulsion}
  \framesubtitle{\textit{EM acceleration with potential for high thrust \& power density with MW-scale potential.}}
    \textbf{Lorentz Force:} $\vec{F} = \vec{J} \times \vec{B}$
    \vspace{0.5em}
    \begin{itemize}
        \item \textbf{MPD (Magnetoplasmadynamic):} 
            \begin{itemize}
                \item Central cathode + annular anode.
                \item High $J_r$ discharge current arc ionizes propellant \& induces strong $B_\theta$ field.
                \item $F = J \times B$ acceleration of plasma in axial direction.
                \item Efficient at high power ($>100$ kW).
            \end{itemize}
            \vspace{0.5em}
        \item \textbf{PPT (Pulsed Plasma Thruster):}
            \begin{itemize}
                \item Oldest form of electric propulsion (1960s).
                \item Capacitor discharge arc across face of Teflon fuel bar between coaxial electrodes.
                \item Ablated plasma accelerated via $J \times B$ force.
                \item Low efficiency ($<15\%$), used for CubeSats/Attitude control.
            \end{itemize}
            \vspace{0.5em}
        \item \textbf{VASIMR - Variable Specific Impulse Magnetoplasma Rocket:}
            \begin{itemize}
                \item RF Ionization (Helicon) + RF Heating (ICRH).
                \item Variable $I_{sp}$ at constant power.
            \end{itemize}
    \end{itemize}
\end{frame}

\iffalse
\begin{frame}{Pulsed Plasma Thruster Schematic}

  \begin{figure}[h]
    \centering
    \begin{tikzpicture}
        % Electrodes
        \draw[thick, fill=gray!50] (0, 1) rectangle (4, 1.5); % Anode
        \draw[thick, fill=gray!50] (0, -1) rectangle (4, -1.5); % Cathode
        \node at (4.2, 1.25) {Anode (+)};
        \node at (4.2, -1.25) {Cathode (-)};
        
        % Teflon Fuel Bar
        \draw[thick, fill=white] (0, -1) rectangle (1, 1);
        \node at (0.5, 0) [rotate=90] {Teflon Fuel};
        \draw[->, thick] (-0.5, 0) -- (0, 0) node[pos=0, left] {Spring Force};
        
        % Capacitor Circuit
        \draw (1, 1.5) -- (1, 2.5) -- (3, 2.5) -- (3, 1.5);
        \draw (1, 2.5) -- (1.8, 2.5);
        \draw (2.2, 2.5) -- (3, 2.5);
        \draw[thick] (1.8, 2.2) -- (1.8, 2.8);
        \draw[thick] (2.2, 2.2) -- (2.2, 2.8);
        \node at (2, 3) {Capacitor};
        
        % Spark Plug
        \draw[thick, red] (1.1, 1.5) -- (1.1, 1.0);
        \node at (1.1, 1.8) [right, red] {Ignitor};
        
        % Plasma Discharge
        \shade[inner color=white, outer color=cyan] (1, -1) rectangle (1.5, 1);
        \node at (1.25, 0.5) {\tiny Arc};
        
        % Lorentz Force
        \draw[->, ultra thick, red] (1.5, 0) -- (3.5, 0) node[right] {$\vec{J} \times \vec{B}$ Force};
        \draw[->, blue, dashed] (2.5, 0.5) -- (2.5, -0.5) node[midway, right] {$\vec{J}$};
        \draw[blue] (2.5, 0) circle (0.1) node {\tiny $\odot$};
        \node at (2.7, 0.2) {\tiny $\vec{B}$};

    \end{tikzpicture}
    \caption{Electromagnetic: Pulsed Plasma Thruster (PPT)}
  \end{figure}
\end{frame}
\fi

\subsection{Micro-Propulsion}
\begin{frame}{Micro-Propulsion: Electrospray}
  \framesubtitle{\textit{CubeSat-scale propulsion}}
  \textbf{Physics:}
  \begin{itemize}
      \item Electrostatic charged particle extraction from conductive liquid surface.
      \item \textbf{Taylor Cone} formation balance (Surface tension vs. Electric stress).
  \end{itemize}
  
  \begin{equation}
      V_{start} \approx \sqrt{\frac{\gamma r_c}{\epsilon_0}} \ln\left(\frac{4d}{r_c}\right)
  \end{equation}
  
  \textbf{Modes:}
  \begin{itemize}
      \item \textbf{Cone-Jet (Colloid):} Charged droplets.
      \item \textbf{Ionic (FEEP/ILIS):} Pure ion evaporation ($I_{sp} > 6000$ s).
  \end{itemize}
  \textbf{Scaling:} MEMS arrays (thousands of emitters) required for useful thrust.
\end{frame}

\iffalse
\begin{frame}{Electrospray Thruster Schematic}

  \begin{figure}[h]
    \centering
    \begin{tikzpicture}[rotate=-90]
        % Emitter (rotated: originally vertical, now horizontal pointing right)
        \draw[thick, fill=gray!20] (0, 0) rectangle (2, 4);
        \node[rotate=0] at (1.0, 1.0) {Reservoir};
        
        % Capillary
        \draw[thick] (0.8, 4) -- (0.8, 5);
        \draw[thick] (1.2, 4) -- (1.2, 5);
        
        % Liquid / Taylor Cone
        \draw[fill=cyan!50] (0.8, 5) -- (1.0, 5.5) -- (1.2, 5) -- cycle;
        \node[rotate=90] at (2.2, 5.5) {Taylor Cone};
        
        % Extractor Grid
        \draw[thick, fill=gray!60] (0, 6) rectangle (0.8, 6.2);
        \draw[thick, fill=gray!60] (1.2, 6) rectangle (2, 6.2);
        \node[rotate=90] at (2.5, 6.1) {Extractor Grid (-V)};
        
        % Jet / Plume
        \draw[thick, cyan] (1.0, 5.5) -- (1.0, 5.8); % Jet
        \foreach \y in {6.5, 7, 7.5} {
            \fill[cyan] (1.0, \y) circle (0.05);
            \fill[cyan] (0.8, \y) circle (0.03);
            \fill[cyan] (1.2, \y) circle (0.03);
        }
        \draw[->, thick] (1.0, 6) -- (1.0, 8) node[rotate=0, right] {Charged Droplets/Ions};

    \end{tikzpicture}
    \caption{Micro-Propulsion: Electrospray Thruster}
  \end{figure}
\end{frame}
\fi

%---------------------------------------------------------
% Section 6: Conclusion
%---------------------------------------------------------
\subsection{Electric Propulsion Summary}

\begin{frame}{Electric Propulsion: Key Takeaways}
  \begin{itemize}
      \item \textbf{Performance:} High $I_{sp}$ \& low thrust density compared to chemical rockets.
      \item \textbf{System Types:}
      \begin{itemize}
          \item Electrothermal (Resistojet, Arcjet).
          \item Electrostatic (Ion, Hall).
          \item Electromagnetic (PPT, MPD).
          \item Micro-propulsion (Electrospray, FEEP).
      \end{itemize}
      \item \textbf{Systems View:} PPU and Thermal management are dominant mass/volume drivers.
      \item \textbf{Future:} Higher power (100 kW+), alternative propellants (Iodine), and micro-scale integration.
  \end{itemize}
\end{frame}

\begin{frame}{Lecture Outline}
  \tableofcontents[currentsection]
\end{frame}

\end{document}